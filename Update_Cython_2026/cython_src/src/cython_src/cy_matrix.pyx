...
cdef class Matrix:
    ...
    cpdef Matrix multiply(self, Matrix other):
        ...
        cdef int i, j, k
        cdef int current_rows = self.matrix_container.num_rows
        cdef int current_cols = self.matrix_container.num_cols
        cdef int other_rows = other.matrix_container.num_rows
        cdef int other_cols = other.matrix_container.num_cols
        if current_cols != other_rows:
            raise ValueError(f'Current cols := {current_cols} != Other rows := {other_rows}.')

        cdef Matrix new_matrix = Matrix()
        new_matrix.matrix_container.num_rows = current_rows
        new_matrix.matrix_container.num_cols = other_cols
        # Allocate memory for the matrix
        new_matrix.matrix_container._c_matrix = <double**> malloc(current_rows * sizeof(double *))
        ...
        for i in range(current_rows):
            new_matrix.matrix_container._c_matrix[i] = <double*>malloc(other_cols*sizeof(double))
        ...
        for i in range(current_rows):
            for j in range(other.matrix_container.num_cols):
                for k in range(self.matrix_container.num_cols):
                    new_matrix.matrix_container._c_matrix[i][j] += (
                                self.matrix_container._c_matrix[i][k] * other.matrix_container._c_matrix[k][j])
        return new_matrix
